...
import { getClassName, getClassColorCode } from "../../utils/helper";
const IspezioniElement2 = ({ areas, handleModal, deleteInspection, showModal, modalRef, setShowModal, TableData2, nestedElements }) => {
    ...
    filterApis.forEach(element => {
        if (element?.table_fields?.field_name?.includes("view")) {
            viewRoute = element?.table_fields?.field_name;
        }
        if (element?.table_fields?.field_name?.includes("edit")) {
            editRoute = element?.table_fields?.field_name;
        }
    });
    ...
    const handleViewClick = (item) => {
        ...
        localStorage.setItem("viewRoute", viewRoute.split("/").pop());
        navigate(`${viewRoute}`);
    };

    const handleEditClick = (item) => {
        localStorage.setItem("ispenzioEditID", item?.id_order);
        localStorage.setItem("editroute", editRoute.split("/").pop());
        localStorage.setItem("order_Data", JSON.stringify({ "order_code": item?.order_code, "client": item?.client }))
        navigate(`${editRoute}`);
    };
        ...
        <>
            <div className="table-responsive">
                <table className="table m b-0">
                    ...
                    <tbody>
                        {TableData2?.data?.map((item,index) => (
                            <tr key={index}>
                                <td>{item?.client}</td>
                                <td>
                                ...
                                <td>
                                    <div className="table_action_list">
                                        ...
                                        <a style={{cursor: "Pointer"}} onClick={() => handleModal(item?.id_order)} className="table_actionBtn">
