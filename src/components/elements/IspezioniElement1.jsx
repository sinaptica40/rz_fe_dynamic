...
const IspezioniElement1 = ({ areas,TableData1 }) => {
    ...
    console.log(addRoute,'check table data here')
    const handleAddRoute =(item)=>{

        console.log(item,'check item data here');
        ...
        localStorage.setItem("addRoute",addRoute?.table_fields?.field_name.split("/").pop())
                        ...
                        <tbody>
                            {TableData1?.data?.map((item)=>(
                                <tr>
                                    ...
                                    {item?.inspections[0] && <td>{item?.inspections[0]?.calendar_info?.date}</td>}
                                    <td>
                                    ...
                                    {item?.ispectors && <td>{item?.ispectors?.map((item, index) => (
                                                    <span key={index}>
                                                        {item}
                                    ...
                                    <div className="btnRight">
                                        <a onClick={()=>handleAddRoute(item)} className="planRightDete">
                                            <img src={addRoute?.table_fields?.label}/> {addRoute?.table_fields?.field_name === "/ispezioni-add" ? "" : "Report"}
                                        </a>
                                    </div>
                                </td>
                                </tr>
