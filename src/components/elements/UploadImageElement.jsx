...
const UploadImageElement = ({ area, userData, handleImageChange }) => {
  return (
    <div className="d-flex align-items-center gap-3">
      {/* Profile Image + Upload Button */}
      <div className="position-relative">
        <img
          src={userData?.profileImage || "/img/default-user.png"} // Fallback image
          alt="Profile"
        ...
        <input
          ...
          style={{ display: "none" }}
          id={`fileInput-${area}`} // Unique ID for each instance
        />
        <label
          htmlFor={`fileInput-${area}`}
      ...
      <div className="fs-4 fw-bold">{userData.username || "User"}</div>
    </div>
  );
};
