...
const MainLayout11 = ({ areas }) => {
  ...
  let getApi = areas
    ...
    .reduce((acc, user) => {
      ...
      if (key?.includes("ReportNavBarArea-5")) {
        acc.getInspectionOrder = functionName;
        acc.getOderApiMethod = api_Method;
      }
      if (key?.includes("ReportNavBarArea2-6")) {
  ...
  // check data here in this section
    ...
    useEffect(() => {
      ...
      const navLinkIndex = sessionStorage.getItem('navlink')
     if(parseInt(navLinkIndex) !== null){
       setPage(parseInt(navLinkIndex))
     }
    }, [])
    ...
    <div className={`app ${isPageOpen ? "is-folded" : ""}`}> 
      ...
      <div className="side_nav">
        ...
        <div className="side_nav_inner">
          {findAreaByKeyPrefix("ReportNavBarArea", { page, setPage })}
          {findAreaByKeyPrefix("ReportNavBarArea2", { inspectionData, getApi,page, setPage})}
        </div>
      </div>
      </div>

      <div className="webcontent-wrapper side_mar">
       {inspectionData && <div>{findAreaByKeyPrefix("ReportComponentArea", { page,  inspectionData, areadata:areas})} </div>}
      </div>
      </div>
    </>
  );
